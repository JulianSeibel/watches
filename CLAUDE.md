...
`watch-overviewnew.html` is the entire project: a single self-contained static page (~5100 lines)
...
1. **Data.** `WATCHES` (line ~709) — 113 object literals, one per watch, ids **1–113, contiguous and
...
2. **Model.** `computeValueScores()` (line ~4243) — turns the data into `specScore`, `valueScore`,
...
|---|---|---|---|
| `EXTRAS` | ~3364 | `id` | lume, warranty, ISO 6425, antimagnetism, clasp, service, bezel, complications, optional `caseScore` override. **All 113 present.** |
| `FINISH` | ~3707 | `id` | 0–1 finishing/decoration estimate. **All 113 present.** |
| `DISPLAY_BACK` | ~3834 | `id` | 1 = see-through, 0 = solid, **absent = not researched**. 103 of 113 researched, 10 open. |
| `WATCH_TYPES` | ~2966 | `id` | array of types (filter only, never scored). **All 113 present.** |
| `WEIGHT_MEASURED` / `WEIGHT_UNPUBLISHED` / `WEIGHT_UNRESOLVED` / `WEIGHT_HEAD_ONLY` | ~2705 / ~2835 / ~2897 / ~2823 | `id` | published grams (51); ids confirmed to publish none (51); ids whose page could not be reached (7); ids published without the band (4). The four are disjoint and together cover all 113 — keep it that way. |
| `MOVEMENT_TIER` | ~3114 | **exact `movementDisplay` string** | 0–1 architecture tier. 61 keys for 61 distinct movements, no misses, no orphans. |
| `MEASURED_ACCURACY` | ~2554 | caliber **substring** of `movementDisplay` | reported real-world rates |
