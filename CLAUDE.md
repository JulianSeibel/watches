...
holding a hand-researched comparison of 178 watches, plus a scoring model that rates each one's
...
Six per-maker traps are worth knowing before you touch those rows. Each is written up in full in
...
- **Henry Archer contradict themselves on the caliber, and it is the same shape as YEMA's** — the
  Tidevand Dacnis (row 178) has a Specifications block saying Miyota 9019 and a DETAILS bullet on
  the same page saying 9015. The two differ by a date module, the maker's own photography shows no
  date window, and the row takes the 9019. Do not "correct" it from the bullet list. See
  `Henry Archer, pass 46`.
- **A Christopher Ward link is one band option out of five, at a ~€200 spread** — and the band
  decides `priceValue`, which weight table figure applies, and `clasp`. Rows 167 and 169 are
  deliberately *not* the SKUs the pass was handed. Resolve the wanted variant through CW's
  Demandware `Product-Variation` endpoint, then re-fetch at the variant's own URL so every field
  is that SKU's; `christopherward.com` refuses a plain scripted fetch but answers `curl` with a
  browser User-Agent. See `Christopher Ward C60 Trident Biscay, pass 46`.
...
```
178 rows · spec 26–66 · sigma 6 · 2 n/s · 3 o/s · 69 published weights
model  LOO RMSE 6.10 vs naive 8.82 · skill 52.3% · 0 sign flip(s) · resample typical 0.29 worst 0.56
```
...
1. **Data.** `WATCHES` — 178 object literals, one per watch, ids **1–178, contiguous and
...
|---|---|---|
| `EXTRAS` | `id` | lume, warranty, ISO 6425, antimagnetism, clasp, service, bezel, complications, optional `caseScore` override. **All 178 present.** |
| `FINISH` | `id` | 0–1 finishing/decoration estimate. **All 178 present.** |
| `DISPLAY_BACK` | `id` | 1 = see-through, 0 = solid, **absent = not researched**. 159 of 178 researched, 19 open. |
| `WATCH_TYPES` | `id` | array of types (filter only, never scored). **All 178 present.** |
...
| `WEIGHT_MEASURED` / `WEIGHT_UNPUBLISHED` / `WEIGHT_UNRESOLVED` / `WEIGHT_HEAD_ONLY` | `id` | published grams (69); ids confirmed to publish none (94); ids whose page could not be reached (8); ids published without the band (7). The four are disjoint and together cover all 178 — keep it that way. |
...
| `MOVEMENT_TIER` | **exact `movementDisplay` string** | 0–1 architecture tier. 89 keys for 89 distinct movements, no misses, no orphans. |
...
- **`PLAIN_TWIN` / `OUT_OF_SCOPE`** — the second refusal, and the sibling of `SUPPORT` above: where
  ...
  measures and the badge reads `o/s` instead of a verdict. Three rows qualify today (134, 135 and
  165 — the last of them the cleanest pair here, one KNIS reference in two dial variants);
