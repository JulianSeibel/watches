...
holding a hand-researched comparison of 160 watches, plus a scoring model that rates each one's
...
1. **Data.** `WATCHES` — 160 object literals, one per watch, ids **1–160, contiguous and
...
|---|---|---|
| `EXTRAS` | `id` | lume, warranty, ISO 6425, antimagnetism, clasp, service, bezel, complications, optional `caseScore` override. **All 160 present.** |
| `FINISH` | `id` | 0–1 finishing/decoration estimate. **All 160 present.** |
| `DISPLAY_BACK` | `id` | 1 = see-through, 0 = solid, **absent = not researched**. 145 of 160 researched, 15 open. |
| `WATCH_TYPES` | `id` | array of types (filter only, never scored). **All 160 present.** |
...
| `WEIGHT_MEASURED` / `WEIGHT_UNPUBLISHED` / `WEIGHT_UNRESOLVED` / `WEIGHT_HEAD_ONLY` | `id` | published grams (63); ids confirmed to publish none (82); ids whose page could not be reached (8); ids published without the band (7). The four are disjoint and together cover all 160 — keep it that way. |
