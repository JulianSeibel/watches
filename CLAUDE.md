...
holding a hand-researched comparison of 151 watches, plus a scoring model that rates each one's
...
```
151 rows · spec 26–66 · sigma 5 · 2 n/s · 2 o/s · 62 published weights
model  LOO RMSE 5.82 vs naive 9.14 · skill 59.4% · 1 sign flip(s) · resample typical 0.32 worst 0.79
```
...
1. **Data.** `WATCHES` — 151 object literals, one per watch, ids **1–151, contiguous and
...
|---|---|---|
| `EXTRAS` | `id` | lume, warranty, ISO 6425, antimagnetism, clasp, service, bezel, complications, optional `caseScore` override. **All 151 present.** |
| `FINISH` | `id` | 0–1 finishing/decoration estimate. **All 151 present.** |
| `DISPLAY_BACK` | `id` | 1 = see-through, 0 = solid, **absent = not researched**. 137 of 151 researched, 14 open. |
| `WATCH_TYPES` | `id` | array of types (filter only, never scored). **All 151 present.** |
...
| `WEIGHT_MEASURED` / `WEIGHT_UNPUBLISHED` / `WEIGHT_UNRESOLVED` / `WEIGHT_HEAD_ONLY` | `id` | published grams (62); ids confirmed to publish none (76); ids whose page could not be reached (8); ids published without the band (5). The four are disjoint and together cover all 151 — keep it that way. |
...
| `MOVEMENT_TIER` | **exact `movementDisplay` string** | 0–1 architecture tier. 79 keys for 79 distinct movements, no misses, no orphans. |
