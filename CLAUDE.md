...
holding a hand-researched comparison of 162 watches, plus a scoring model that rates each one's
...
```
162 rows · spec 26–66 · sigma 6 · 2 n/s · 2 o/s · 64 published weights
model  LOO RMSE 6.22 vs naive 9.03 · skill 52.6% · 0 sign flip(s) · resample typical 0.34 worst 0.71
```
...
1. **Data.** `WATCHES` — 162 object literals, one per watch, ids **1–162, contiguous and
...
|---|---|---|
| `EXTRAS` | `id` | lume, warranty, ISO 6425, antimagnetism, clasp, service, bezel, complications, optional `caseScore` override. **All 162 present.** |
| `FINISH` | `id` | 0–1 finishing/decoration estimate. **All 162 present.** |
| `DISPLAY_BACK` | `id` | 1 = see-through, 0 = solid, **absent = not researched**. 147 of 162 researched, 15 open. |
| `WATCH_TYPES` | `id` | array of types (filter only, never scored). **All 162 present.** |
...
| `WEIGHT_MEASURED` / `WEIGHT_UNPUBLISHED` / `WEIGHT_UNRESOLVED` / `WEIGHT_HEAD_ONLY` | `id` | published grams (64); ids confirmed to publish none (83); ids whose page could not be reached (8); ids published without the band (7). The four are disjoint and together cover all 162 — keep it that way. |
