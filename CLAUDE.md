...
holding a hand-researched comparison of 154 watches, plus a scoring model that rates each one's
...
model  LOO RMSE 5.94 vs naive 9.14 · skill 57.8% · 1 sign flip(s) · resample typical 0.30 worst 0.63
```
...
1. **Data.** `WATCHES` — 154 object literals, one per watch, ids **1–154, contiguous and
...
|---|---|---|
| `EXTRAS` | `id` | lume, warranty, ISO 6425, antimagnetism, clasp, service, bezel, complications, optional `caseScore` override. **All 154 present.** |
| `FINISH` | `id` | 0–1 finishing/decoration estimate. **All 154 present.** |
| `DISPLAY_BACK` | `id` | 1 = see-through, 0 = solid, **absent = not researched**. 140 of 154 researched, 14 open. |
| `WATCH_TYPES` | `id` | array of types (filter only, never scored). **All 154 present.** |
...
| `WEIGHT_MEASURED` / `WEIGHT_UNPUBLISHED` / `WEIGHT_UNRESOLVED` / `WEIGHT_HEAD_ONLY` | `id` | published grams (62); ids confirmed to publish none (79); ids whose page could not be reached (8); ids published without the band (5). The four are disjoint and together cover all 154 — keep it that way. |
...
| `MOVEMENT_TIER` | **exact `movementDisplay` string** | 0–1 architecture tier. 80 keys for 80 distinct movements, no misses, no orphans. |
