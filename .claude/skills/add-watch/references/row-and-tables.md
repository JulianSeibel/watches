...
| **ueeshop** | Jianghun, pass 42 | The whole spec table renders server-side as `l_i_title` / `l_i_content` pairs, so `--probe` returns caliber, all three case dimensions, L2L, water resistance, glass, lume, crown, caseback, band, taper, clasp and weight in one request. **The price is the trap**: the storefront is geo-aware, so a German IP gets `priceCurrency` EUR in the schema.org offer AND a euro figure in `cur_price`, both converted from a USD base at a fixed in-shop rate. `ueeshop_config.currency_rate` names that rate beside the currency; refetch the same URL with `?currency=USD` to get `currency_rate = '1.0000'` and the base price. Cookies do not switch it, the query parameter does. Variant options carry a `data="{"Price":0,...}"` attribute each, so a zero means the option adds nothing to the price |
| **Salesforce Commerce Cloud (Demandware)** | christopherward.com, pass 46 | The whole Technical table renders server-side — but `page-text.js` cannot see it, because the site refuses the script's plain `fetch` outright (`fetch failed`, every path). `curl` with a browser `-A` answers on the first attempt with ~380 KB. **That is a script limitation, not a finding about the maker**: do not record anything as unpublished on the strength of it. The real trap is the band. One reference is sold across five band options at a ~€200 spread, and the URL pins exactly one — band decides `priceValue`, which weight figure applies, and `clasp`. Resolve a different one through `Product-Variation?dwvar_<pid>_WStrapColourMaterialType=<band>&dwvar_<pid>_WDialBezelColour=<colour>&pid=<pid>` (grep the page for `Product-Variation?` to get the `pid` and the exact parameter names, which vary per model), which returns JSON with the variant's own SKU id and price — then **re-fetch that SKU's own page**, because weight and strap material differ per band. Price also sits in the schema.org offer |
